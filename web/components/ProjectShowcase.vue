  ...
  <section class="project-showcase" data-reveal>
    ...
    <div class="grid gap-6 md:grid-cols-3">
      <article v-for="project in projects" :id="project.slug" :key="project.slug" class="project-card group relative overflow-hidden rounded-3xl border border-border/60 bg-card/70 p-5 shadow-sm backdrop-blur" data-reveal>
        ...
        <div class="relative">
          <div class="flex items-start justify-between gap-4"><div><h3 class="text-lg font-extrabold">{{ project.title }}</h3><p class="mt-2 text-sm text-muted">{{ project.description }}</p></div><span class="badge">{{ project.tag }}</span></div>
          <div class="project-preview mt-4 flex aspect-[4/3] items-center justify-center rounded-2xl bg-muted/60 ring-1 ring-border/60"><span class="material-symbols-outlined text-3xl text-blue-500" aria-hidden="true">{{ project.icon }}</span></div>
          <ul class="mt-4 space-y-2 text-sm text-muted"><li v-for="highlight in project.highlights" :key="highlight" class="flex gap-2"><span aria-hidden="true" class="material-symbols-outlined text-base text-emerald-500">check</span>{{ highlight }}</li></ul>
